...
describe Mallow::DSL do
  ...
  describe '#to' do
    it "appends an action to the current rule" do
      ...
      @dsl.actions.should have(1).thing
    end
  end

  describe '#a' do
    it "appends a class-matching condition" do
      cond = @dsl.a(Array).conditions.last
  ...
  describe '#size' do
    it "appends a size-matching condition" do
      cond = @dsl.size(1).conditions.last
      [[1], {a: :b}].each do |e|
  ...
  describe '#where' do
    it 'appends an arbitrary condition' do
      @dsl.where {12345}.conditions.last.call.should == 12345
    end
  end
